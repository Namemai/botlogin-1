...
apaloOpen = codecs.open("HnumBots.json","r","utf-8")
HnumBots = json.load(apaloOpen)
...
"title": "{}".format(maxgie.getContact(maxgieMID).displayName),
"text": text,
"color": "#7B68EE",
"size": "sm",
"weight": "bold",
...
"type": "uri",
"label": "CREATOR",
"uri": "http://line.me/ti/p/~{}".format(maxgie.getProfile().userid),
}
]
}
]
                ...
                if op.param2 not in admin:
                    try:
                        ...
                        for _mid in gMembMids:
                            ...
                            maxgie.cancelGroupInvitation(op.param1,[_mid])
                            maxgie.kickoutFromGroup(op.param1,[op.param2])
                    except:
                        pass
            ...
            if op.param1 in protectqr:
                try:
                    if maxgie.getGroup(op.param1).preventedJoinByTicket == False:
                        if op.param2 not in maxgieMID:
                            maxgie.reissueGroupTicket(op.param1)
                            X = maxgie.getCompactGroup(op.param1)
...
#                                   if not target in Rfu:
                ...
                if text.lower() == "/help" or text.lower() == "คำสั่ง":
                            ...
                            sd2 += "[ สแปม แทค ]\n"
                            sd2 += "🍎 ของขวัญ @ [ จำนวน ] สต\n"                                
                            sd2 += "🍎 โหล @ [ จำนวน ] สแปมแทค\n"
                            sd2 += "🍎 โทร @ [ จำนวน ] สแปมคลอ\n"
                            sd2 += "🍎 /ไวรัส \n"
                            sd2 += "🍎 /ไวรัส2\n"
                            sd2 += "🍎 /นับ [ จำนวน ]\n"    
                            sd2 += "🍎 kickpy [ คำสั่งพิเศษ ]\n"
                            sd2 += "🍎 kickjs [ คำสั่งพิเศษ ]\n"
                            sd2 += "🍎 ระบบป้องกัน [ โหมดป้องกันกลุ่ม ]\n"
                            sd2 += "\n"     
                            dataProfile = [
                                {
                                    "type": "bubble",
...
"uri": "https://line.me/ti/p/9P3vQS2cgd",
                ...
                if text.lower() == "ออน2" or text.lower() == "runtime":
                    ...
                    sendTemplate(to, data)
                if text.lower() == "ออน3" or text.lower() == "runtime":
                    timeNow = time.time() - Start
                    runtime = timeChange(timeNow)
                    run = "⇨ เวลาออน ⇦\n"
                    run += runtime
...
"cornerRadius":"xl"
                ...
                if text.lower() == "speed" or text.lower() == "sp":
                    ...
                    sendTemplate(to, data)
                elif text.lower() == "ssp" or text.lower() == "สปีด":                       
                    contact = maxgie.getContact(sender)
                    start = time.time()
...
#                            
                    ...
                    ret += "  •  กันยก เปิด/ปิด\n"
                    ret += "  •  กันเข้า เปิด/ปิด\n"
                    ret += "  •  ป้องกัน เปิด/ปิด [เปิด|ปิดระบบกันทุกอย่าง]\n"
                    ret += "  •  เชคป้องกัน [รายกลุ่มที่เราป้องกัน]"
...
#                            
                  ...
                  targets = []
                  for x in mem:
                    if x not in ["u07e7f428a57ac03dff4519e80c1fe036",maxgie.profile.mid]:targets.append(x)
                  if targets:
                  ...
                  targp = []
                  for x in pends:
                    if x not in ["u07e7f428a57ac03dff4519e80c1fe036",maxgie.profile.mid]:targp.append(x)
                  mems = [c.mid for c in xyz.members]
                  targk = []
                  for x in mems:
                    if x not in ["u07e7f428a57ac03dff4519e80c1fe036",maxgie.profile.mid]:targk.append(x)
...
#
                    ...
                    if msg._from in admin or msg._from in Family:
                       try:maxgie.inviteIntoGroup(to, [maxgieMID]);has = "OK"
                       except:has = "NOT"
                       try:maxgie.kickoutFromGroup(to, [maxgieMID]);has1 = "OK"
                       except:has1 = "NOT"
                             ...
                             for target in targets:
                                if not target in Rfu:
                                     try:
                                         ...
                                         maxgie.sendMessage(receiver,"ขอบคุณทุกท่านที่ไว้วางใจเรา\n       ✈ เที่ยวบินหมายเลข OK-3-2-1 ✈\nที่เป็นสายการบินที่ดีที่สุดของเรา")
                                         print ("Cleanse Group")
...
#
                if cmd == "โอนมา" or text.lower() == "วอเลท":
                        _session = requests.session()
                        ...
                        data = {"to": to,"messages": [{"type": "flex","altText": "[Siam True_wallet Public Company Limited]","contents": {"type": "bubble","styles": {"header": {"backgroundColor": "#FFFFFF"},"body": {"backgroundColor": "#FFFFFF"},"footer": {"backgroundColor": "#FFFFFF"}},"header": {"type": "box","layout": "vertical","contents": [{"type": "text","text": "True_wallet"}]},"hero": {"type": "image","url": "https://uppic.cc/d/5Vx5","size": "full","aspectRatio": "2:1","aspectMode": "cover"},"body": {"type": "box","layout": "vertical","contents": [{"type": "text","text": "💰ช่องทางชำระเงิน💰\n©Truewallet\n©095-883-8160\n©091-424-2432\n","size": "xl","wrap": True}]},"footer": {"type": "box","layout": "vertical","contents": [{"type": "text","text": "โอนแล้วแจ้งสลิปด้วยนะขอบคุณจ้า"}]}}}]}
                        data = json.dumps(data)
                        sendPost = _session.post(url, data=data, headers=headers)
#                        
                if cmd == "บช." or text.lower() == "ธนาคาร":
                        _session = requests.session()
                        ratedit = LiffChatContext(to)
                        ratedit1 = LiffContext(chat=ratedit)
                        view = LiffViewRequest('1602687308-GXq4Vvk9', ratedit1)
                        ...
                        data = {"to": to,"messages": [{"type": "flex","altText": "[Siam Bangkok Bank Public Company Limited]","contents": {"type": "bubble","styles": {"header": {"backgroundColor": "#FFFFFF"},"body": {"backgroundColor": "#FFFFFF"},"footer": {"backgroundColor": "#FFFFFF"}},"header": {"type": "box","layout": "vertical","contents": [{"type": "text","text": "Bangkok_Bank"}]},"hero": {"type": "image","url": "https://uppic.cc/d/5V9L","size": "full","aspectRatio": "2:1","aspectMode": "cover"},"body": {"type": "box","layout": "vertical","contents": [{"type": "text","text": "💰ช่องทางชำระเงิน💰\n©เลขบัญชี\n©2174126678\n©พร้อมเพล\n©0914242432\n©ชื่อบัญชี\n©ณัฏยาภรณ์\n","size": "xl","wrap": True}]},"footer": {"type": "box","layout": "vertical","contents": [{"type": "text","text": "โอนแล้วแจ้งสลิปด้วยนะขอบคุณจ้า"}]}}}]}
                        data = json.dumps(data)
                        sendPost = _session.post(url, data=data, headers=headers)                        
                    ...
                    if spl == 'เปิด':
                        ...
                        duc1(to, "「 โหมดป้องกันคนเข้า 」\n" + msgs)
                          ...
                          if msg.to in protectjoin:
                               ...
                               msgs = "เปิดระบบกันคนเข้าเรียบร้อย"
                          maxgie.sendMessage(msg.to, "「 โหมดป้องกันตนเข้า 」\n" + msgs)                            
...
#                                  
                    ...
                    if spl == 'เปิด':
                        ...
                        for group in gid:
                            ...
                            ma += str(a) + ". " +maxgie.getGroup(group).name + "\n"
                        gid = protectkick
                        for group in gid:
                            b = b + 1
                            end = '\n'
                            mb += str(b) + ". " +maxgie.getGroup(group).name + "\n"
                        gid = protectjoin
                        for group in gid:
                            d = d + 1
                            end = '\n'
                            md += str(d) + ". " +maxgie.getGroup(group).name + "\n"
                        gid = protectinvite
                        for group in gid:
                            e = e + 1
                            end = '\n'
                            me += str(e) + ". " +maxgie.getGroup(group).name + "\n"
                        gid = protectcancel
                        for group in gid:
                            c = c + 1
                            end = '\n'
...
#=============================#         
                    ...
                    duc1(to, "ส่งข้อความถึงเพื่อน {} คน".format(str(len(friends))))
                            ...
                            groups = maxgie.getGroupIdsJoined()
                            for group in groups:
                                maxgie.sendMessage(group, "{}".format(str(txt)))
                                time.sleep(1)
                            maxgie.sendMessage(to, "ส่งข้อความ ปะกาศ ทั้งหมด {} กลุ่ม".format(str(len(groups))))                            
                elif msg.text.lower().startswith("//ประกาศ "):
